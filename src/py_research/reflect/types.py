...
def is_subtype[S: TypeForm | UnionType](
    type_: TypeForm | UnionType, supertype: S
) -> TypeGuard[S]:
    ...
    if not isinstance(type_, TypeAliasType):
        ...
        if len(type_args) == 0 and len(supertype_args) == 0:
            ...
            supertype_tuple = tuple(typedef_to_typeset(cast(TypeForm, supertype)))
            return all(issubclass(t, supertype_tuple) for t in typeset)

        return is_subhint(type_, supertype)  # pyright: ignore[reportArgumentType]

    return is_subhint(type_.__value__, supertype)  # pyright: ignore[reportArgumentType]


def has_type(obj: Any, type_: TypeForm[T]) -> TypeGuard[T]:
    """Check if object is of given type hint."""
...
def extract_nullable_type(type_: TypeForm[T | None]) -> type[T] | None:
    """Extract the non-none base type of a union."""
    args = get_args(type_)

    if len(args) == 0 and has_type(type_, type):
        return type_
...
def typedef_to_typeset(
    typedef: TypeForm | UnionType | None,
    ...
    typedef_set: set[TypeForm | UnionType | None] = {typedef}
...
class TypeRef[T]:
    ...
    hint: TypeForm | ForwardRef | str | TypeVar = cast(type[T], object)
    ...
    def base_type(self, bound: type[T] | None = None) -> type[T] | None:
        ...
        if has_type(self.hint, TypeForm):
    ...
    @property
    def single_typedef(self) -> TypeForm[T]:
        """Return the type format."""
        return (
            self.typeform
